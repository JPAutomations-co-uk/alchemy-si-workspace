...
def run(payload: dict, token_data: dict, slack_notify=None) -> dict:
    ...
    if not reply:
        ...
        log_activity(
            "autoreply", "item_processed",
            f"Skipped reply to {payload.get('lead_email', 'unknown')} — conversation complete",
            metrics={"action": "skip", "campaign_id": campaign_id},
            status="success"
        )
    ...
    if "error" in result:
        ...
        log_activity(
            "autoreply", "run_error",
            f"Failed to send reply to {payload.get('lead_email', 'unknown')}",
    ...
    log_activity(
        "autoreply", "item_processed",
        f"Replied to {payload.get('lead_email', 'unknown')}",
        metrics={"reply_length": len(reply), "campaign_id": campaign_id},
        status="success"
    )
    return {
        "status": "success",
