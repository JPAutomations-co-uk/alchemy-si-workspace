...
image = (
    ...
    .add_local_dir("/Users/jp/VS Code Workspace/directives", remote_path="/app/directives")
    .add_local_dir("/Users/jp/VS Code Workspace/execution", remote_path="/app/execution")
    .add_local_file("/Users/jp/VS Code Workspace/execution/webhooks.json", remote_path="/app/webhooks.json")
    .add_local_file("/Users/jp/VS Code Workspace/.tmp/demo_kickoff_call_transcript.md", remote_path="/app/demo_kickoff_call_transcript.md")
    .add_local_file("/Users/jp/VS Code Workspace/.tmp/demo_sales_call_transcript.md", remote_path="/app/demo_sales_call_transcript.md")
)
...
ALL_SECRETS = [
    ...
    modal.Secret.from_name("mailerlite-secret"),
]
...
# Adds subscriber to MailerLite with segment + custom fields
# ============================================================================

def mailerlite_get_or_create_group(name: str, headers: dict) -> str:
    """Return the MailerLite group ID for a given name, creating it if needed."""
    import requests
    ...
    for group in resp.json().get("data", []):
        if group["name"] == name:
            return group["id"]
    resp = requests.post("https://connect.mailerlite.com/api/groups", headers=headers, json={"name": name}, timeout=10)
...
def jp_training_lead(body: dict):
    ...
    Called from book.html after Web3Forms fires.
    Adds subscriber to MailerLite with segment tag + all quiz answers as custom fields.
    """
    import requests
    ...
    # Slack notification for every lead regardless of email
    slack_notify(
        f"📋 *JP Training Lead*\n"
        f"Name: {name} | Phone: {phone} | Email: {email or 'none'}\n"
        f"Age: {age_raw} → *{segment}* | Goals: {goals_str} | Timeline: {timeline}"
    )

    # Can't add to MailerLite without an email
    if not email:
        return JSONResponse({"status": "ok", "note": "no_email_skipped_mailerlite"})

    # Get or create the segment group
    group_id = mailerlite_get_or_create_group(segment, headers)
