...
def poll_bulk_search_status(search_id):
    ...
    MAX_ITERATIONS = 72  # 72 × 10s = 12 minutes max
    iteration = 0
    ...
    while iteration < MAX_ITERATIONS:
        ...
        try:
            ...
            if status == "completed":
                ...
                print(f"⏳ Status: {status} - {processed}/{total} rows processed... [{iteration}/{MAX_ITERATIONS}]")
                time.sleep(10)
            else:
                print(f"Status: {status} [{iteration}/{MAX_ITERATIONS}]")
                time.sleep(10)
    ...
    print(f"⏰ Bulk search timed out after {MAX_ITERATIONS * 10}s — skipping")
    return False
