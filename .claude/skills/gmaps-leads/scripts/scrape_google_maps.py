...
def scrape_google_maps(
    ...
    # UK city coordinates for accurate geolocation
    UK_CITIES = {
    ...
    # Set country and coordinates for UK searches
    if location:
        location_lower = location.lower().strip()
        run_input["countryCode"] = "gb"
        for city_name, coords in UK_CITIES.items():
            if city_name in location_lower:
                run_input["lat"] = str(coords["lat"])
                run_input["lng"] = str(coords["lng"])
                run_input["zoom"] = 12
                print(f"Using UK coordinates for {location}: {coords['lat']}, {coords['lng']}")
                break
