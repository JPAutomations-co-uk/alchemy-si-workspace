...
def main():
    ...
    try:
        ...
        try:
            worksheet = spreadsheet.worksheet("Qualified Leads")
        except Exception:
    ...
    # Find column indices
    try:
        # Support both generic and gmaps-leads schema column names
        email_idx = headers.index("emails") if "emails" in headers else headers.index("email")
        first_name_idx = headers.index("owner_name") if "owner_name" in headers else headers.index("first_name")
        company_name_idx = headers.index("business_name") if "business_name" in headers else headers.index("company_name")
